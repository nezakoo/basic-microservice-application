...
# Queues to store request timestamps and durations
request_times = deque()
...
# Create fibonacci payload with random number
@app.route('/payload', methods=['GET'])
def payload():
    n = random.randint(1, 10000)  # Generate a random integer between 1 and 10000
    fib = fibonacci(n)
...
def update_average_duration():
    now = time.time()
    # Filter and keep durations from the last 60 seconds
